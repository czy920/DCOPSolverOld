...
public class AlsMusAgent extends AgentCycleAls{
	...
	public AlsMusAgent(int id, String name, int level, int[] domain) {
	...
	private void sendStep1Messages(){
		if(myIdentity == NONE){
			...
			for(int neighbourIndex = 0; neighbourIndex < neighboursQuantity; neighbourIndex++){
				Message msg = new Message(this.id, neighbours[neighbourIndex], AlsMusAgent.TYPE_STEP1_MESSAGE, box);
	...
	private void sendStep2Messages(){
		if(myIdentity == NONE){
			...
			box[0] = valueIndex;
			for(int neighbourIndex = 0; neighbourIndex < neighboursQuantity; neighbourIndex++){
				Message msg = new Message(this.id, neighbours[neighbourIndex], AlsMusAgent.TYPE_STEP2_MESSAGE, box);
				this.sendMessage(msg);
			}
		}
		else if(myIdentity == SUGGESTER){
	...
	private void sendStep3Messages(){
		if(myIdentity == NONE || myIdentity == SUGGESTER || myIdentity == ACCEPTER2){
			...
			box[0] = valueIndex;
			for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
				Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMusAgent.TYPE_STEP3_MESSAGE, box);
				this.sendMessage(msg);
			}
		}
	...
	protected void disposeMessage(Message msg) {
		...
		if(msg.getType() == AlsMusAgent.TYPE_STEP1_MESSAGE){
			disposeStep1Message(msg);
		}
		else if(msg.getType() == AlsMusAgent.TYPE_STEP2_MESSAGE){
			disposeStep2Message(msg);
		}
		else if(msg.getType() == AlsMusAgent.TYPE_STEP3_MESSAGE){
			disposeStep3Message(msg);
		}
		//!!!!!!!!!!!!!!!!!!!!要添加disposeAlsCostMessage(msg)的处理模块!!!!!!!!!!!!!!!!!!!!
		else if(msg.getType() == AlsMusAgent.TYPE_ALSCOST_MESSAGE){
			disposeAlsCostMessage(msg);
		}
		//!!!!!!!!!!!!!!!!!!!!要添加disposeAlsBestMessage(msg)的处理模块!!!!!!!!!!!!!!!!!!!!
		else if(msg.getType() == AlsMusAgent.TYPE_ALSBEST_MESSAGE){
