...
//	private int[] neighboursAreaCostGA;
	...
	protected void initRun() {
		...
		pCross = Settings.settings.getSelectNewProbability();
...
//			neighboursAreaCostGA[i] = 2147483647;
...
//				min = i;
//		}
//		box[0] = localCostGA[min];
//		box[1] = (localCostGA[min]*1000/neighboursQuantity);
		for(int i = 0; i< population; i++){
			box[i+2] = valueIndexGA[i];
		}
...
//		neighboursAreaCostGA[senderIndex] = ((int[])(msg.getValue()))[0];
//		neighboursLocalCostGA[senderIndex] =  ((int[])(msg.getValue()))[1];
		if(((int[])(msg.getValue()))[0] == 1)
			crossTag = true;
		for(int i = 0; i < population; i++){
			neighboursValueIndexGA[i][senderIndex] = ((int[])(msg.getValue()))[i+2];
		}
