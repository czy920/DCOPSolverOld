...
public class DsaSDP_Agent extends AgentCycleAls {
	...
	public DsaSDP_Agent(int id, String name, int level, int[] domain) {
	...
	private void sendValueMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
			Message msg=new Message(this.id, neighbours[neighbourIndex], DsaSDP_Agent.TYPE_VALUE_MESSAGE, valueIndex);
	...
	public String easyMessageContent(Message msg, AgentCycle sender,
			AgentCycle receiver) {
		// TODO 自动生成的方法存根
		return "from "+sender.getName()+" to "+receiver.getName()+" type "+DsaSDP_Agent.messageContent(msg);
	}
	

	public static String messageContent(Message msg){
		switch (msg.getType()) {
		case DsaSDP_Agent.TYPE_VALUE_MESSAGE:
		{
