...
public class DsaAgent extends AgentCycle {
	...
	private void sendValueMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
			Message msg=new Message(this.id, neighbours[neighbourIndex], DsaAgent.TYPE_VALUE_MESSAGE, valueIndex);
	...
	protected void disposeMessage(Message msg) {
		...
		if(receivedQuantity==0){
			...
			cycleCount++;
			System.out.println(this.name+" at "+cycleCount);
	...
	public String easyMessageContent(Message msg, AgentCycle sender,
			AgentCycle receiver) {
		// TODO Auto-generated method stub
		return "from "+sender.getName()+" to "+receiver.getName()+" type "+DsaAgent.messageContent(msg);
	}

	
	public static String messageContent(Message msg){
		switch (msg.getType()) {
		case DsaAgent.TYPE_VALUE_MESSAGE:
		{
