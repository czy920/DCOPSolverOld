...
public class DsaC_Agent extends AgentCycle {
	...
	public DsaC_Agent(int id, String name, int level, int[] domain) {
	...
	private void sendValueMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
			Message msg=new Message(this.id, neighbours[neighbourIndex], DsaC_Agent.TYPE_VALUE_MESSAGE, valueIndex);
	...
	public String easyMessageContent(Message msg, AgentCycle sender,
			AgentCycle receiver) {
		// TODO Auto-generated method stub
		return "from "+sender.getName()+" to "+receiver.getName()+" type "+DsaC_Agent.messageContent(msg);
	}

	
	public static String messageContent(Message msg){
		switch (msg.getType()) {
		case DsaC_Agent.TYPE_VALUE_MESSAGE:
		{
