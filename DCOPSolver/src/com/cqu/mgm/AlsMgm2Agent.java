...
import com.cqu.dsa.AlsDsa_H1_Agent;
import com.cqu.main.Debugger;
import com.cqu.settings.Settings;

public class AlsMgm2Agent extends AgentCycleAls {
	...
	public AlsMgm2Agent(int id, String name, int level, int[] domain) {
	...
	private void sendValueMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
			Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_VALUE_MESSAGE, valueIndex);
			this.sendMessage(msg);
		}
	}
	
	private void sendOfferMessages(){
		Message msg=new Message(this.id, neighbours[coordinate], AlsMgm2Agent.TYPE_OFFER_MESSAGE, selectValueGroup);
		this.sendMessage(msg);
	}
	
	private void sendWaitMessages(int neighbourIndex){
		Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_WAIT_MESSAGE, valueIndex);
		this.sendMessage(msg);
	}
	
	private void sendAcceptMessages(int[] temp){
		Message msg=new Message(this.id, neighbours[coordinate], AlsMgm2Agent.TYPE_ACCEPT_MESSAGE, temp);
		this.sendMessage(msg);
	}
	
	private void sendRejectMessages(int neighbourIndex){
		Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_REJECT_MESSAGE, valueIndex);
		this.sendMessage(msg);
	}
	
	private void sendWaitGainMessages(int neighbourIndex){
		Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_WAITGAIN_MESSAGE, valueIndex);
		this.sendMessage(msg);
	}
	...
	private void sendGainMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity ; neighbourIndex++){
			if(neighbourIndex!=coordinate){
				Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_GAIN_MESSAGE, gainValue);
				this.sendMessage(msg);
			}
		}
	}
	...
	private void sendCoGainMessages(){
		Message msg=new Message(this.id, neighbours[coordinate], AlsMgm2Agent.TYPE_COGAIN_MESSAGE, valueIndex);
		this.sendMessage(msg);
	}
	private void sendDecideGoMessages(){
		Message msg=new Message(this.id, neighbours[coordinate], AlsMgm2Agent.TYPE_DECIDEGO_MESSAGE, isAbleToGo);
		this.sendMessage(msg);
	}
	...
	private void sendWaitAgainMessages(){
		for(int neighbourIndex=0; neighbourIndex<neighboursQuantity; neighbourIndex++){
			if(neighbourIndex!=coordinate){
				Message msg=new Message(this.id, neighbours[neighbourIndex], AlsMgm2Agent.TYPE_WAITAGAIN_MESSAGE, valueIndex);
				this.sendMessage(msg);
			}
		}
	}
	...
	*/
	...
	protected void disposeMessage(Message msg) {
		...
		*/
		...
		if(msg.getType()==AlsMgm2Agent.TYPE_VALUE_MESSAGE)
		{
			disposeValueMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_OFFER_MESSAGE)
		{
			disposeOfferMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_WAIT_MESSAGE)
		{
			disposeWaitMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_ACCEPT_MESSAGE)
		{
			disposeAcceptMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_REJECT_MESSAGE)
		{
			disposeRejectMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_WAITGAIN_MESSAGE)
		{
			disposeWaitGainMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_GAIN_MESSAGE)
		{
			disposeGainMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_COGAIN_MESSAGE)
		{
			disposeCoGainMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_DECIDEGO_MESSAGE)
		{
			disposeDecideGoMessage(msg);
		}else if(msg.getType()==AlsMgm2Agent.TYPE_WAITAGAIN_MESSAGE)
		{
			disposeWaitAgainMessage(msg);
		}else if(msg.getType() == AlsMgm2Agent.TYPE_ALSCOST_MESSAGE)
		{
			disposeAlsCostMessage(msg);
		}
		else if(msg.getType() == AlsMgm2Agent.TYPE_ALSBEST_MESSAGE)
		{
	...
	public static String messageContent(Message msg){
		switch (msg.getType()) {
		case AlsMgm2Agent.TYPE_VALUE_MESSAGE :
			int val=(Integer) msg.getValue();
			int valueIndex=val;
			return "value["+valueIndex+"]";
			
		case AlsMgm2Agent.TYPE_OFFER_MESSAGE :
			return "offer[ ]";
			
		case AlsMgm2Agent.TYPE_WAIT_MESSAGE :
			return "wait[ ]";
			
		case AlsMgm2Agent.TYPE_ACCEPT_MESSAGE :
			return "accept[ ]";
			
		case AlsMgm2Agent.TYPE_REJECT_MESSAGE :
			return "reject[ ]";
			
		case AlsMgm2Agent.TYPE_WAITGAIN_MESSAGE:
			return "wait_gain[ ]";
			
		case AlsMgm2Agent.TYPE_GAIN_MESSAGE :
			int gainValue=(Integer) msg.getValue();
			return "gain["+gainValue+"]";
			
		case AlsMgm2Agent.TYPE_COGAIN_MESSAGE :
			return "co_gain[ ]";
			
		case AlsMgm2Agent.TYPE_DECIDEGO_MESSAGE :
			return "decide_go["+(String)(msg.getValue())+"]";
			
		case AlsMgm2Agent.TYPE_WAITAGAIN_MESSAGE :
			return "wait_again[ ]";
