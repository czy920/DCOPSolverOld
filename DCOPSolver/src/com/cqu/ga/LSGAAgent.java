...
public class LSGAAgent extends MALSAgentCycle {
    ...
    public LSGAAgent(int id, String name, int level, int[] domain) {
    ...
    private void crossover() {
        ...
        int temp  = valueIndexList[0];
        for (int i = POPULATION - 1; i > 0; i--) {
            valueIndexList[i] = tempList[i-1];
        }
        valueIndexList[POPULATION - 1] = temp;
        mutation();
    }

    private void sendCrossoverMessage() {
        for (int neighborId : neighbours) {
            Message msg = new Message(this.id,neighborId,TYPE_CROSS_MESSAGE,null);
            sendMessage(msg);
        }
    }
