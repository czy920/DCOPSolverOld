...
public class LSGAMGM2roulette extends MALSAgentCycle {
    ...
    public final static int TYPE_LEVEL_MESSAGE = 207;
    public final static int TYPE_CROSS_MESSAGE = 208;
    ...
    private int[] rouletteP = new int[POPULATION];
    private int sumCost;
    private HashMap<Integer,Integer> levelView = new HashMap<>();


    public LSGAMGM2roulette(int id, String name, int level, int[] domain) {
    ...
    protected void allMessageDisposed() {
        ...
        if (cycleCount<=CYCLE_COUNT_END){
            ...
            if (cycleCount % (CROSS_CYCLE) == 0){
                ...
                System.out.println("crossoverCycleCount: " + cycleCount);
    ...
    private void cycleForOffer() {//选一个最优的offer对接
        if (isCommitted){//接收到offer消息的节点
            ...
            for (int i = 0; i < POPULATION; i++) {
                tempGain[i] = gain[i];
                isChanged[i] = false;
                if (offerViewGroup.get(coordinate)!=null){
