...
public class MeetingSchedulingParser extends ContentParser{
	
	public MeetingSchedulingParser(Problem problem, Element root,
	...
	public void parseContent() {
		...
		List<Variable> variables=parseVariables(root.getChild(NodeNames.VARIABLES));
		if(variables==null)
		{
		...
		for(Variable var : variables)
		{
			...
			problem.putNodeDomain(id, var.getDomainName());
		...
		for(int i=0;i<agentIds.length;i++)
		{
			for(int j=i+1;j<agentIds.length;j++)
			{
				...
				if(sameAttendeeCount>0)
				{
					...
					problem.putConstraintValues(name, costs);
		...
		for(Integer agentId : agentConstraints.keySet())
		{
			...
			problem.putNodeConstraints(agentId, constraints);
			problem.putNodeNeighbours(agentId, neighbors);
		}
	}
	...
	private List<Variable> parseVariables(Element element)
	{
		...
		for(int i=0;i<nbVariables;i++)
		{
			...
			String domainName=elementList.get(i).getAttributeValue(NodeNames.DOMAIN);
			String agentName=elementList.get(i).getAttributeValue(NodeNames.AGENT);
