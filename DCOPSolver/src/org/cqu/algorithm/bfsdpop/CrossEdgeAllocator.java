...
public class CrossEdgeAllocator {
	...
	public CrossEdgeAllocator(Problem problem) {
		...
		{
			int[] neighbours=pseudotreeProblem.getNodeNeighbours(agentId);
			boolean[] crossNeighbours=new boolean[neighbours.length];
			boolean[] considered=new boolean[neighbours.length];
			Integer parent=pseudotreeProblem.getNodeParent(agentId);
			int[] children=pseudotreeProblem.getNodeChildren(agentId);
	...
	public void allocate()
	{
		...
		while(edgeList.size()>0)
		{
			for(int i=0;i<edgeList.size();i++)
			{
				...
				int index=CollectionUtil.indexOf(problem.getNodeNeighbours(edge.getNodeB()), edge.getNodeA());
