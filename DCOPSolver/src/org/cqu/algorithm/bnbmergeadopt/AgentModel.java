...
//		}
	...
	public class AdoptMethod extends Method {
		...
		protected void sendCostMessage() {
			...
			cost.put(SAKeys.CONTEXT, new Context(context));
			cost.put(SAKeys.LB, agent.LB);
			cost.put(SAKeys.UB, agent.UB);

			Message msg = new Message(agent.getId(), agent.parent,
					MessageTypes.COST, cost);
...
//				thresh.put(AgentModel.CONTEXT,
//						new Context(agent.currentContext));
//				thresh.put(AgentModel.TH,
...
//						AgentModel.VALUE,val);

				Map<String, Object> mapValue = new HashMap<String, Object>();
//				Context c = new Context(agent.currentContext);
//				c.addOrUpdate(agent.id, agent.valueIndex, agent.valueID);
//				mapValue.put(AgentModel.CONTEXT, c);
//				mapValue.put(AgentModel.VALUE_MESSAGE, valueMsg);

				Message msg = new Message(agent.getId(), childId,
						MessageTypes.TERMINATE, mapValue);
				agent.sendMessage(agent.constructNcccMessage(msg));
			}
...
//			if (((Context) thresh.get(AgentModel.CONTEXT))
//					.compatible(agent.currentContext) == true) {
//				agent.TH = (Integer) thresh.get(AgentModel.TH);
//				maintainThresholdInvariant();
//				backtrack();
//			}
//		}
...
//						.get(AgentModel.CONTEXT);
//				valueMsg = (Message) mapValue.get(AgentModel.VALUE_MESSAGE);
...
//			System.out.println("agent"+agent.id+": "+agent.valueIndex+"\t"+agent.valueID+"\t"+agent.TH+"\t"+agent.LB+"\t"+agent.UB+"\t"+agent.nccc);
		...
		protected void sendCostMessage() {
			...
			cost.put(SAKeys.CONTEXT, context);
			cost.put(SAKeys.LB, agent.LB);
			cost.put(SAKeys.UB, agent.UB);
...
//				mapValue.put(AgentModel.VALUE_MESSAGE, valueMsg);
		...
		protected void disposeTerminateMessage(Message msg) {
			...
			// msg.getValue();
			// currentContext = (Context) mapValue.get(CONTEXT);
			// //不应该加入里，而要自己去处理判断。
			// valueMsg = (Message) mapValue.get(VALUE_MESSAGE);
