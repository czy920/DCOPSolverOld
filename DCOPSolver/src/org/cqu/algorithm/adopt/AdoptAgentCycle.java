package org.cqu.algorithm.adopt;
...
import org.cqu.common.Infinity;
import org.cqu.common.ResultAdopt;
import org.cqu.common.constant.keys.SAKeys;
import org.cqu.core.control.Message;
import org.cqu.core.control.MessageNCCC;
import org.cqu.core.control.MessageTypes;
import org.cqu.core.control.synchronous.AgentCycle;
import org.cqu.gui.settings.Settings;


public class AdoptAgentCycle extends AgentCycle{
	...
	private void sendValueMessages()
	{
		...
		for(int i=0;i<this.allChildren.length;i++)
		{
			...
			Message msg=new Message(this.id, childId, MessageTypes.VALUE, valueIndex);
	...
	private void sendCostMessage()
	{
		...
		cost.put(SAKeys.CONTEXT, new Context(currentContext));
		cost.put(SAKeys.LB, LB);
		cost.put(SAKeys.UB, UB);
		
		Message msg=new Message(this.id, this.parent, MessageTypes.COST, cost);
		this.sendMessage(this.constructNcccMessage(msg));
	}
	...
	private void sendThresholdMessages()
	{
		...
		for(int i=0;i<this.children.length;i++)
		{
			...
			thresh.put(SAKeys.CONTEXT, new Context(currentContext));
			thresh.put(SAKeys.TH, this.ths.get(childId)[valueIndex]);
			
			Message msg=new Message(this.id, childId, MessageTypes.THRESHOLD, thresh);
			this.sendMessage(this.constructNcccMessage(msg));
		}
	}
	...
	private void sendTerminateMessages()
	{
		...
		for(int i=0;i<this.children.length;i++)
		{
			...
			Message valueMsg=new Message(this.id, childId, MessageTypes.VALUE, valueIndex);
			
			Map<String, Object> mapValue=new HashMap<String, Object>();
			mapValue.put(SAKeys.CONTEXT, c);
			mapValue.put(SAKeys.VALUE_MESSAGE, this.constructNcccMessage(valueMsg));
			
			Message msg=new Message(this.id, childId, MessageTypes.TERMINATE, mapValue);
			this.sendMessage(this.constructNcccMessage(msg));
		}
		...
		if(this.pseudoChildren!=null)
		{
			...
			for(int i=0;i<this.pseudoChildren.length;i++)
			{
				...
				Message valueMsg=new Message(this.id, pseudoChildId, MessageTypes.VALUE, valueIndex);
				
				Message msg=new Message(this.id, pseudoChildId, MessageTypes.TERMINATE, this.constructNcccMessage(valueMsg));
				this.sendMessage(this.constructNcccMessage(msg));
			}
		}
	}
	...
	/*private void sendTerminateMessages()
	{
		...
		for(int i=0;i<this.children.length;i++)
		{
			...
			Message msg=new Message(this.id, childId, AdoptAgent.TERMINATE, c);
	...
	protected void disposeMessage(Message msg) {
		...
		}else if(type==MessageTypes.COST)
		{
			disposeCostMessage(msg);
		}else if(type==MessageTypes.THRESHOLD)
		{
			disposeThresholdMessage(msg);
		}else if(type==MessageTypes.TERMINATE)
		{
	...
	protected void messageLost(Message msg) {
		// TODO Auto-generated method stub
		if(msg.getType()==MessageTypes.TERMINATE)
		{
			sendMessage(msg);
		}
		if(Debugger.debugOn==true)
	...
	private void disposeCostMessage(Message msg)
	{
		...
		Context c=(Context) cost.get(SAKeys.CONTEXT);
		...
		{
			lbs.get(msg.getIdSender())[myValueIndex]=(Integer) cost.get(SAKeys.LB);
			ubs.get(msg.getIdSender())[myValueIndex]=(Integer) cost.get(SAKeys.UB);
	...
	private void disposeThresholdMessage(Message msg)
	{
		...
		if(((Context)thresh.get(SAKeys.CONTEXT)).compatible(currentContext)==true)
		{
			this.TH=(Integer) thresh.get(SAKeys.TH);
			maintainThresholdInvariant();
			backtrack();
		}
	}
	...
	private void disposeTerminateMessage(Message msg)
	{
		...
		if(msg.getIdSender()==this.parent)
		{
			...
			currentContext=(Context) mapValue.get(SAKeys.CONTEXT);
			valueMsg=(Message) mapValue.get(SAKeys.VALUE_MESSAGE);
	...
	protected void runFinished() {
		...
		result.put(SAKeys.ID, this.id);
		result.put(SAKeys.NAME, this.name);
		result.put(SAKeys.VALUE, this.domain[valueIndex]);
		result.put(SAKeys.LB, this.LB);
		result.put(SAKeys.UB, this.UB);
		result.put(SAKeys.TH, this.TH);
		result.put(SAKeys.NCCC, this.nccc);
	...
	public Object printResults(List<Map<String, Object>> results) {
		...
		{
			int id_=(Integer) result.get(SAKeys.ID);
			String name_=(String) result.get(SAKeys.NAME);
			int value_=(Integer) result.get(SAKeys.VALUE);
			int LB_=(Integer) result.get(SAKeys.LB);
			int UB_=(Integer) result.get(SAKeys.UB);
			int TH_=(Integer) result.get(SAKeys.TH);
			int ncccTemp=(Integer) result.get(SAKeys.NCCC);
			if(maxNccc<ncccTemp)
			{
				maxNccc=ncccTemp;
			}
	...
	public static String messageContent(Message msg)
	{
		...
		case MessageTypes.COST:
		{
			Map<String, Object> msgValue=(Map<String, Object>) msg.getValue();
			int LB_=(Integer) msgValue.get(SAKeys.LB);
			int UB_=(Integer) msgValue.get(SAKeys.UB);
			Context c=(Context) msgValue.get(SAKeys.CONTEXT);
			return "cost[LB="+LB_+" UB="+Infinity.infinityEasy(UB_)+" context="+c.toString()+"]";
		}
		case MessageTypes.THRESHOLD:
		{
			Map<String, Object> msgValue=(Map<String, Object>) msg.getValue();
			int TH_=(Integer) msgValue.get(SAKeys.TH);
			Context c=(Context) msgValue.get(SAKeys.CONTEXT);
