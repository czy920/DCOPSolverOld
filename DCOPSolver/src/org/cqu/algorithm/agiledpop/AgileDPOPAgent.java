package org.cqu.algorithm.agiledpop;
...
public class AgileDPOPAgent extends TreeNodeAgent{
	...
	public AgileDPOPAgent(int id, String name, int[] domain, int[] neighbours,
	...
	private void mergeAndReduct(List<MultiDimensionData> allDatas)
	{
		...
		while(reductable==true)
		{
			...
			{
				if(allDatasDimensions.get(i).getPriority()<this.layer)
				{
	...
	private void sendValueMessage(Map<Integer, Integer> valueIndexes)
	{
		...
		{
			Message valueMsg=new Message(this.id, this.children[i], VALUE, CollectionUtil.copy(valueIndexes));
