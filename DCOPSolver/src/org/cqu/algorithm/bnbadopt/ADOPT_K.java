package org.cqu.algorithm.bnbadopt;
...
public class ADOPT_K extends AgentCycle {
	...
	//public final static String VALUE_MESSAGE = "VALUE_MESSAGE";
...
//		System.out.println("agent"+this.id+": "+this.valueIndex+"\t"+this.valueID+"\t"+this.TH_A+"\t"+this.TH_B+"\t"+this.LB+"\t"+this.UB);
	...
	private void sendCostMessage() {
		...
		cost.put(ADOPTKKeys.CONTEXT, context);
		cost.put(ADOPTKKeys.LB, LB);
		cost.put(ADOPTKKeys.UB, UB);
	...
	private void sendValueMessages() {
		...
		for(int i=0;i<this.children.length;i++)
		{
			...
			Message msg=new Message(this.id, childId, MessageTypes.VALUE, val);
			this.sendMessage(this.constructNcccMessage(msg));
		}
		}
		...
		for(int i=0;i<this.pseudoChildren.length;i++)
		{
			...
			Message msg=new Message(this.id, pseudoChildId, MessageTypes.VALUE, val);
			this.sendMessage(this.constructNcccMessage(msg));
			}
		}
	...
	public Object printResults(List<Map<String, Object>> results) {
		...
		for (Map<String, Object> result : results) {
			int id_ = (Integer) result.get(ADOPTKKeys.ID);
			String name_ = (String) result.get(ADOPTKKeys.NAME);
			int value_ = (Integer) result.get(ADOPTKKeys.VALUE);
			int LB_ = (Integer) result.get(ADOPTKKeys.LB);
			int UB_ = (Integer) result.get(ADOPTKKeys.UB);
			int TH_A = (Integer) result.get(ADOPTKKeys.TH_A);
			int TH_B = (Integer) result.get(ADOPTKKeys.TH_B);
			int ncccTemp = (Integer) result.get(ADOPTKKeys.NCCC);
	...
	protected void runFinished() {
		...
		result.put(ADOPTKKeys.ID, this.id);
		result.put(ADOPTKKeys.NAME, this.name);
		result.put(ADOPTKKeys.VALUE, this.domain[valueIndex]);
		result.put(ADOPTKKeys.LB, this.LB);
		result.put(ADOPTKKeys.UB, this.UB);
		result.put(ADOPTKKeys.TH_A, this.TH_A);
		result.put(ADOPTKKeys.TH_B, this.TH_B);
		result.put(ADOPTKKeys.NCCC, this.nccc);
	...
	public static String messageContent(Message msg) {
		...
		case MessageTypes.COST: {
			...
			int LB_ = (Integer) msgValue.get(ADOPTKKeys.LB);
			int UB_ = (Integer) msgValue.get(ADOPTKKeys.UB);
			Context c = (Context) msgValue.get(ADOPTKKeys.CONTEXT);
	...
	private void disposeCostMessage(Message msg) {
		...
		Context c = (Context) cost.get(ADOPTKKeys.CONTEXT);
