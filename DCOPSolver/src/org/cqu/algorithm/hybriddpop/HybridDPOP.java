...
import org.cqu.algorithm.hybridmbdpop.ContextWrapped;
import org.cqu.algorithm.hybridmbdpop.UtilMessage;
...
public class HybridDPOP extends TreeNodeAgent{
	...
	public HybridDPOP(int id, String name, int[] domain, int[] neighbours,
			MessageSink msgSink, StatisticsListener statisticsListener,
			int capacity, boolean isSearchingPolicy, boolean[] neighbourPolicy, ContextWrapped contextWrapped) {
	...
	private void startFromLeaf()
	{
		...
		if(isNeighborSearchingPolicy.get(this.parent)==true)
		{
			mdData=mdData.shrinkDimension(this.parent+"", this.contextWrapped.getValueIndex(this.parent));
		}
	...
	private void computeLocalUtils()
	{
		for(int i=0;i<allParents.length;i++)
		{
			...
			dimensions.add(new Dimension(parentId+"", row, this.neighbourLayers.get(parentId)));
		...
		for(int i=0;i<pseudoChildren.length;i++)
		{
			...
			dimensions.add(new Dimension(this.id+"", this.domain.length, this.layer));
			dimensions.add(new Dimension(pseudoChildId+"", row, this.neighbourLayers.get(pseudoChildId)));
	...
	public Object printResults(List<Map<String, Object>> results) {
		...
		for(int i=0;i<results.size();i++)
		{
			...
			int id_=(Integer) result.get(ID);
			String name_=(String) result.get(NAME);
			int value_=(Integer) result.get(VALUE);
			if(result.containsKey(KEY_TOTAL_COST))
			{
		...
		contextContent=contextContent.substring(0, contextContent.length()-1);
		contextContent+="}";
		ret.otherResults.put("ContextSize", this.contextWrapped.size()+contextContent);
		System.out.println("ContextSize: "+this.contextWrapped.size()+contextContent);
	...
	{
		switch (msg.getType()) {
		case HybridDPOPMessageTypes.VALUE:
		{
			int valueIndex=(Integer) msg.getValue();
			return "value["+valueIndex+"]";
		}
		case HybridDPOPMessageTypes.UTILITY:
		{
	...
	protected void disposeMessage(Message msg) {
		...
		if(type==HybridDPOPMessageTypes.VALUE)
		{
			disposeValueMessage(msg);
		}else if(type==HybridDPOPMessageTypes.UTILITY)
		{
			utilMsgSizes.add(((UtilMessage) msg).getMdData().size());
			disposeUtilMessage(msg);
		}else if(type==HybridDPOPMessageTypes.NEXTITERATION)
		{
			disposeNextIterationMessage(msg);
		}else if(type==HybridDPOPMessageTypes.ITERATIONOVER)
		{
	...
	private void sendUtilMessage(MultiDimensionData multiDimentionalData)
	{
		...
		Message utilMsg=new UtilMessage(this.id, this.parent, HybridDPOPMessageTypes.UTILITY, multiDimentionalData, this.contextWrapped);
		...
		if(receivedMDDatas.size()>=this.children.length)
		{
			...
			if(this.isSearchingPolicy==true)
			{
				...
				if(this.isRootAgent()==true)
				{
					if(iterationOver==true)
					{
						this.totalCost=mdData.getData()[0];
						this.valueIndex=this.bestContext.getValueIndex(this.id);
	...
	private void sendValueMessage(Map<Integer, Integer> valueIndexes)
	{
		...
		{
			Message valueMsg=new Message(this.id, this.children[i], HybridDPOPMessageTypes.VALUE, CollectionUtil.copy(valueIndexes));
	...
	private void sendNextIterationMessage()
	{
		...
		for(int i=0;i<this.children.length;i++)
		{
			Message nextIterationMsg=new Message(this.id, this.children[i], HybridDPOPMessageTypes.NEXTITERATION, null);
	...
	private void sendIterationOverMessage()
	{
		...
		for(int i=0;i<this.children.length;i++)
		{
			Message nextIterationMsg=new Message(this.id, this.children[i], HybridDPOPMessageTypes.ITERATIONOVER, this.contextWrapped);
			this.sendMessage(nextIterationMsg);
		}
	}
